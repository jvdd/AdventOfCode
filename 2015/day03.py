...
## Part 1
def moveSanta(grid, pos, direction):
	if direction == '>':
	...
	if direction == 'v':
		if pos[0] + 1 >= len(grid):
			grid = grid + [[0] * len(grid[pos[0]])]
		pos = [pos[0] + 1,pos[1]]

	if direction == '^':
		if pos[0] - 1 < 0:
			grid = [[0] * len(grid[pos[0]])] + grid
		else:
			pos = [pos[0] -1, pos[1]]
...
for direction in directions:
	(grid,pos) = moveSanta(grid,pos,direction)

nbHousesVisited = sum(map(sum,grid))
print(nbHousesVisited)


## Part 2
def moveSantaAndRobo(grid, pos, otherPos, direction):
	if direction == '>':
		if len(grid[pos[0]]) <= pos[1] + 1:
			for row in grid:
				row.append(0)
	...
	if direction == '<':
		if pos[1] - 1 < 0:
			for row in grid:
				row.insert(0,0)
			otherPos = [otherPos[0],otherPos[1] + 1]
		else:
			pos = [pos[0],pos[1] - 1]
	...
	if direction == '^':
		if pos[0] - 1 < 0:
			grid = [[0] * len(grid[pos[0]])] + grid
			otherPos = [otherPos[0] + 1, otherPos[1]]
		else:
			pos = [pos[0] -1, pos[1]]
	
	grid[pos[0]][pos[1]] = 1
	return (grid,pos,otherPos)

grid = [[1]]
pos1 = [0,0]
pos2 = [0,0]
for index in range(0,len(directions),2):
	(grid,pos1,pos2) = moveSantaAndRobo(grid,pos1,pos2,directions[index])
	(grid,pos2,pos1) = moveSantaAndRobo(grid,pos2,pos1,directions[index + 1])
...
# NOTE
# It is important to realize that the position of the robo-santa, or the casual santa
# (just the other santa who isn't at move) also may need to change.
# These changes are necessary when the (infinite) grid grows upwards or to the left.
# This is because I choose my axis points [0,0] in the upper left corner.
# By going down we increase the first coordinate, going right increases the second
# coordinate. But going left and means means that we need to respectively add a colum 
# to the left or insert a new row at the top. Hence, the coordinates of the other 
# santa do not point anymore to the same position (as intended), and thus need to
# be changed.
